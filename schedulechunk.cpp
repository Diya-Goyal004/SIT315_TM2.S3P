...
int main() {
    ...
    // Adding parallelism using OpenMP with different scheduling techniques and chunk sizes
    #pragma omp parallel default(none) shared(v1, v2, v3, size) reduction(+:total)
    {
        int private_total = 0; // Private variable to store partial sum for each thread

        // Generate random values for v1 and v2 in parallel
        randomVector(v1, size);
        randomVector(v2, size);

        // Parallelize the loop for vector addition with different scheduling techniques and chunk sizes
        // Experiment with different scheduling types and chunk sizes
        #pragma omp for schedule(static,10000) 
        for (int i = 0; i < size; i++) {
            v3[i] = v1[i] + v2[i]; // Add corresponding elements of v1 and v2 and store in v3
            private_total += v3[i]; // Update private total for each thread
        }
